...
.glow-red-strong {
  box-shadow: 0 0 16px rgba(130, 20, 0, 0.7), 0 0 32px rgba(130, 20, 0, 0.3);
}
.glow-gold {
  box-shadow: 0 0 8px #e6b820, 0 0 16px rgba(230, 184, 32, 0.4);
}
.text-glow-red {
  text-shadow: 0 0 8px rgba(130, 20, 0, 0.9);
}
.text-glow-gold {
  text-shadow: 0 0 8px #e6b820, 0 0 16px rgba(230, 184, 32, 0.4);
}
```
  ...
  "sections": [
    ...
    },
    {
      ...
      "data": {
        ...
        "steps": [
          { "name": "xs",   "size": "10px", "tracking": "3px",  "weight": 400, "sample": "SYSTEM LABEL" },
          { "name": "sm",   "size": "12px", "tracking": "2px",  "weight": 400, "sample": "Navigation item" },
          { "name": "base", "size": "14px", "tracking": "1px",  "weight": 400, "sample": "Body copy reads here" },
          { "name": "lg",   "size": "18px", "tracking": "2px",  "weight": 700, "sample": "Card Title" },
          { "name": "xl",   "size": "24px", "tracking": "3px",  "weight": 900, "sample": "SECTION HEADING" },
          { "name": "2xl",  "size": "36px", "tracking": "4px",  "weight": 900, "sample": "VISUALISER" }
        ]
      }
    }
  ]
...
describe('carouselStore', () => {
  ...
  it('does not exceed total - 1', () => {
    store.next(); store.next(); store.next(); store.next();
...
export function createCarouselStore(total: number) {
  ...
  const state = derived(index, ($i): CarouselState => ({
    activeIndex: $i,
    total,
 ...
 */
export function animateCarouselChange(refs: {
  activeCard: Element;
  ghostCards: Element[];
}) {
  const tl = gsap.timeline({ defaults: { ease: 'power2.inOut', duration: 0.35 } });

  tl.to(refs.activeCard,  { opacity: 1, y: 0 })
    .to(refs.ghostCards,  { opacity: 0.35, y: 0 }, '<');
 ...
 */
export function animateProjectEntry(refs: {
  sidebar: Element;
  topbar: Element;
  content: Element;
}) {
 ...
 */
export function animateProjectExit(refs: {
  sidebar: Element;
  topbar: Element;
  content: Element;
}) {
...
<script lang="ts">
  ...
  function handleKey(e: KeyboardEvent) {
    if (e.key === 'ArrowDown') { e.preventDefault(); carousel.next(); }
    if (e.key === 'ArrowUp')   { e.preventDefault(); carousel.prev(); }
  }
...
<style>
  ...
  .sigil {
    ...
    box-shadow: 0 0 20px rgba(130, 20, 0, 0.5), inset 0 0 20px rgba(130, 20, 0, 0.1);
    display: flex;
    align-items: center;
    justify-content: center;
  ...
  .hero-divider {
    ...
    box-shadow: 0 0 8px rgba(130, 20, 0, 0.8), 0 0 16px rgba(130, 20, 0, 0.3);
    margin: 16px 0;
  }
...
<script lang="ts">
  ...
  onMount(async () => {
    [projects, project] = await Promise.all([
      listProjects(),
      getProject(slug),
    ]);
  ...
  <div class="swatches">
    {#each swatches as swatch}
      <div class="swatch" class:glow={swatch.glow}>
        <div
          class="swatch-color"
          style="background: {swatch.hex}; {swatch.glow ? `box-shadow: 0 0 12px ${swatch.hex}55` : ''}"
        ></div>
...
<style>
  .colour-palette { margin-bottom: 48px; }
  ...
  .swatch-name  { font-size: 11px; color: var(--color-text); letter-spacing: 1px; }
  .swatch-hex   { font-size: 10px; color: var(--color-muted); }
  .swatch-token { font-size: 9px;  color: var(--color-ghost); letter-spacing: 1px; }
</style>
```
...
<style>
  ...
  .step-name   { font-size: 9px; color: var(--color-red); letter-spacing: 2px; min-width: 28px; }
  .step-size   { font-size: 9px; color: var(--color-muted); min-width: 36px; }
  .step-track  { font-size: 9px; color: var(--color-ghost); min-width: 36px; }
  .step-weight { font-size: 9px; color: var(--color-ghost); }

  .step-sample { color: var(--color-text); flex: 1; }
</style>
```
